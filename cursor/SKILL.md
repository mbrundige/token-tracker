...
```bash
~/.cursor/skills/token-tracker/scripts/set-token-context.js --workspace "$PWD" --feature "<name>"
```

Or to clear:

```bash
~/.cursor/skills/token-tracker/scripts/set-token-context.js --workspace "$PWD" --clear-feature
```
